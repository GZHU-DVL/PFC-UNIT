...
def natural_sort(items):
    items.sort(key=natural_keys)#排序
...
class ImageMaskLabelDataset(data.Dataset):
    def initialize(self, imgroot, maskroot, files, dataset_sizes, labels, 
        ...
        for i in range(len(files)):
            ...
            dataset_size = min(dataset_sizes[i], len(image_path))#CelebAMask-HQ原数据集有30000，只取29000;synImageNet291数据集600
            
            mask_path = []
            for j in range(dataset_size):
                mask_name = image_path[j].replace(imgroot, maskroot, 1)
                if 'CelebA-HQ-mask' in mask_name:
                    mask_path += [os.path.splitext(mask_name)[0] + '.png']
                else:
