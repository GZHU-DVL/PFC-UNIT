...
class TestOptions():
    def __init__(self):
        ...
        self.parser = argparse.ArgumentParser(description="Inference of PFC-UNIT")
        self.parser.add_argument("--content", type=str, default='./data/afhq/val/dog/flickr_dog_000572.jpg', help="path to the content image")
        ...
        self.parser.add_argument("--output_type", type=str, default='before', help="before or after")
    def parse(self):
...
if __name__ == "__main__":
    ...
    if args.style is not None:
        save_image(I_yhat[0].cpu(), os.path.join(args.output_path, args.output_type+'_'+save_name+'.jpg'))
        save_image(torchvision.utils.make_grid(torch.cat([Ix, Iy, I_yhat.cpu()], dim=0), 3, 2), 
               os.path.join(args.output_path, args.output_type+'_'+save_name+'_overview.jpg'))
    else:
        for i in range(args.batch):
            save_image(I_yhat[i].cpu(), os.path.join(args.output_path, args.output_type+'_'+save_name+'_%d'%(i)+'.jpg'))
